...
public class JogoCenario extends CenarioPadrao {
    ...
    private PontoJogador pontoA, pontoB;
    ...
    private final Texto textoPausa = new Texto(PontoJogador.fonte);
    ...
    public JogoCenario(int largura, int altura) throws IOException {
        ...
        pontoA = new PontoJogador();
        pontoB = new PontoJogador();
    }
    ...
    public void carregar() {
        ...
        pontoA.setPy(PontoJogador.TAMANHO_FONTE);

        pontoB.setPx(largura / 2 + 120 - PontoJogador.TAMANHO_FONTE / 2);
        pontoB.setPy(PontoJogador.TAMANHO_FONTE);
    ...
    public void desenhar(Graphics2D g) {
        ...
        if (Jogo.pausado)
            textoPausa.desenha(g, "PAUSA", largura / 2 - PontoJogador.TAMANHO_FONTE, altura / 2);

    }


    private boolean validaColisao(Bola b) throws IOException {
        boolean saiu = false;
        ...
        if (Util.colide(esquerda, b)) {
            ...
            if (b.getPx() < 0) { //Pontuar jogador B
                pontoB.addPonto();
                if (pontoB.getPontos() - pontoA.getPontos() == 3) {
                    pontoA.setJogadorNome(JOptionPane.showInputDialog("Nome do Jogador 1"));
                    pontoB.setJogadorNome(JOptionPane.showInputDialog("Nome do Jogador 2"));

                    try {
                        new Rank(pontoA, pontoB); //Rank em execução
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                ...
                System.out.println("Jogador B = " + pontoB.getPontos()); //bola saindo
            } else { //Pontuar Jogador A
                pontoA.addPonto();
                if (pontoA.getPontos() - pontoB.getPontos() == 3) {
                    pontoA.setJogadorNome(JOptionPane.showInputDialog("Nome do Jogador 1"));
                    pontoB.setJogadorNome(JOptionPane.showInputDialog("Nome do Jogador 2"));
                ...
                System.out.println("Jogador A = " + pontoA.getPontos()); //bola saindo
            }
